...
if (!url) {
    if (onVercel) {
        console.error(
            '[migrate] ERROR: SUPABASE_DB_URL no está configurada en Vercel.',
        )
        console.error(
            '[migrate] Configurala en Project Settings → Environment Variables.',
        )
        console.error(
            '[migrate] Usa el pooler (puerto 6543) para evitar problemas IPv4/IPv6:',
        )
        console.error(
            '[migrate]   postgresql://postgres.PROJECT:PASSWORD@aws-0-REGION.pooler.supabase.com:6543/postgres',
        )
        process.exit(1)
    }
    console.log(
        '[migrate] SUPABASE_DB_URL no está configurada — saltando migraciones (modo local).',
    )
    process.exit(0)
}

// Quitamos sslmode del query string para que no sobreescriba la opción ssl
// del cliente (Supabase usa un cert que pg en modo verify-full rechaza).
